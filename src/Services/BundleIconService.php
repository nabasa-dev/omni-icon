...
class BundleIconService
{
    ...
    public function __construct()
    {
        ...
        $this->registry = new LocalSvgIconRegistry(
            $this->svg_dir,
            [
                'omni' => $this->svg_dir, // Register 'omni' prefix as alias
            ]
        );
    }
    ...
    public function search_icons(string $query): array
    {
        ...
        return array_values(array_map(
            static fn($icon) => ['name' => $icon['name'], 'prefix' => 'omni'],
            array_filter($all_icons, static fn($icon) => str_contains($icon['name'], $search_term))
        ));
    }
}
