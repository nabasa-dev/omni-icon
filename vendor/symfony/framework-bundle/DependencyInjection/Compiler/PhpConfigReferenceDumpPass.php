...
use OmniIconDeps\Symfony\Component\Config\Definition\ArrayShapeGenerator;
...
use OmniIconDeps\Symfony\Component\DependencyInjection\Loader\Configurator\AppReference;
use OmniIconDeps\Symfony\Component\HttpKernel\Bundle\BundleInterface;
use OmniIconDeps\Symfony\Component\Routing\Loader\Configurator\RoutesReference;
/**
 * @internal
 */
...
PHPDOC;
    public function __construct(private string $referenceFile, private array $bundlesDefinition)
    {
    }
    public function process(ContainerBuilder $container): void
    {
        ...
        foreach ($extensionsPerEnv as $env => $extensions) {
            $appTypes = substr_replace($appTypes, strtr(self::WHEN_ENV_APP_TEMPLATE, ['{ENV}' => $env, '{SHAPE}' => $this->getShapeForExtensions($extensions, $container, '    ')]), $i, 0);
        }
        ...
        $configReference = strtr(self::REFERENCE_TEMPLATE, ['{APP_TYPES}' => $appTypes, '{APP_PARAM}' => $appParam, '{ROUTES_TYPES}' => $routesTypes, '{ROUTES_PARAM}' => $r->getMethod('config')->getDocComment()]);
