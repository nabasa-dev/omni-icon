 ...
 */
namespace OmniIconDeps\Symfony\Component\Config\Definition;
...
final class ArrayShapeGenerator
{
    ...
    {
        $name = $node->getName();
        $quoted = str_starts_with($name, '@') || \in_array(strtolower($name), ['int', 'float', 'bool', 'null', 'scalar'], \true) || strpbrk($name, '\'"');
        if ($quoted) {
